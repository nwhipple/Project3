...
// Section 2 on Nuclear Power Plants Dashboard // based on code from class exercise
// This section was assisted extensively by the tutor, espeically in getting map to render properly, helping get the click to pop up info bubbles
let reactorlist = "./reactorlist.geoJSON";
// const ReactorCircle = L.CircleMarker.extend({
//   options:{
//     id:"feature.properties.GEMunitID"
//   }
// })
d3.json(reactorlist).then(function (data) {
  createFeatures(data.features);
  $("#map").on("click",".leaflet-interactive",function(event){
...
function createFeatures(reactorMetaData) {
  function onEachFeature_func(feature, layer) {
    layer.bindPopup(`
      ...
      <hr>
      <p>Capacity MW: ${feature.properties["Capacity (MW)"]}</p>
      <p>Start Year: ${feature.properties["Start Year"]}</p>
      <p>Status: ${feature.properties["Status"]}</p>
      `);
  }
  function markerFunction(feature, latlng) {
    // const marker = new ReactorCircle(latlng)
    const marker = L.circleMarker(latlng)
    // console.log(
    // marker.setAttribute("data-id",feature.properties.GEMunitID)
    // )

    return marker;
  }
  // this section based on leaflet chloropleth tutorial https://leafletjs.com/examples/choropleth/
...
//// Section 1 on Average waste per kWh by source of energy //This code was based off of code used in a class exercise
let title = 'Average pounds of waste per MWh by energy source. Does not include CO2 Emissions.';

let Source = ["Coal", "Solar", "Hydropower", "Nuclear"];

let WasteAmount = [195.8, 3.674, 0.352, 0.066];
...
//// Section on reactor counts per year
// let launchyear = ["Coal", "Natural Gas", "Solar", "Hydropower", "Nuclear"];

// let ReactorCounts = [1850, 970, 32, 28, 0.007];
...
// Plotly.newPlot("plot5", data2, layout2);

//// Second Section on Reactor Cost Data // this section was based on code from a class exercise
////The tutor helped quite a bit on getting this data to render properly and linking the data to the drop-down menu
...
d3.json("nuclearincidents.json").then(data => {
  ...
  L.geoJSON(geojsonFeatures, {
      ...
      onEachFeature: function(feature, layer) {
          layer.bindPopup(`<b>${feature.properties.Incident}</b><br>${feature.properties.Country}<br>INES Level: ${feature.properties['INES level']}<br>IAEA description: ${feature.properties['IAEA description']}<br>${feature.properties.Year}`);
      }
  }).addTo(map);
})
